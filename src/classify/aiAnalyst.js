...
`;
  ...
  // 核心高级运营文案生成器 (支持定制和引流)
  async generateCustomOperationsCopy(repoOrRepos, type, { tone, audience, customPrompt, cta, wechatName = 'GitHub 搞钱雷达' }) {
    const dsKeyCheck = this.deepseekApiKey || process.env.DEEPSEEK_API_KEY;
    if (!dsKeyCheck) {
      return this.generateMockCustomOperationsCopy(repoOrRepos, type, { tone, audience, customPrompt, cta, wechatName });
    }
...
Step 2：结合上述爆款规律和该开源项目的功能亮点，构思吸睛的小红书文案。严禁通篇营销废话，必须明确大白话介绍工具用途。文末必须包含引流钩子："因为平台不让放链接，工具的免安装打包版和一键运行教程，我已经完整整理好了。大家可以去微信看我的同名公众号：【${wechatName}】，回复暗号 ‘[此处填写AI拟定的暗号]’ 就能直接获取啦！"
...
Step 2：标题发散。必须打破千篇一律的格式，构思 3 条【风格完全迥异】的标题（微信爆文标题通常在 20-30 字左右）：
  - 标题A【情绪痛点型】：直击目标受众最大的焦虑或痛点。
  - 标题B【反常识猎奇型】：打破常规认知，制造极强的点击好奇心。
  - 标题C【直白干货型】：数字先行，强调极高价值的“资源/教程”属性。
Step 3：长文创作。以爆款结构为骨架，写一篇 1500 字左右的深度图文，融合 Step 0.5 的优势，必须给出小白极速运行指南，结尾处自然引入资源下载。

文末必须包含以下固定下载结构：
"📥 资源下载与免安装版获取
* 百度网盘: https://pan.baidu.com/s/mock_download_link
* 阿里云盘: https://www.aliyundrive.com/s/mock_download_link"
...
`;
      ...
      console.log('【AI一鱼两吃】正在并行发起 DeepSeek 小红书和微信公众号文案生成...');
...
请对以下 GitHub 热门项目与海外（YouTube/Hacker News）、国内（B站/V2EX）社交媒体热点进行深度比对与量化评分。

【GitHub 热门项目列表】
${JSON.stringify(formattedRepos, null, 2)}

【多源社媒热门话题列表 (包含 YouTube, Hacker News, V2EX, Bilibili)】
...
[
  {
    "repo_name": "项目名称",
    "github_score": 数字(0-100基于stars等热度),
    "social_score": 数字(0-100基于在YouTube/Hacker News等海外平台的热度),
    "domestic_score": 数字(0-100基于在Bilibili/V2EX等国内平台的热度),
    "info_gap": 数字(信息差指数，计算公式：(github_score + social_score) / 2 - domestic_score，分值越高说明海外爆火但国内还没人发),
    "trend_direction": "↑或→或↓",
    "verdict": "强推或关注或观望" (优先强推 info_gap 高的项目),
    "social_match": "简短说明匹配情况",
    "xhs_copy": "如果 info_gap > 10 且 verdict 为强推，请用 emoji 和爆款网感为它生成一段 50 字左右的小红书引流笔记文案，否则填 null"
  }
]
## PART_1_JSON_END
...
### 1. 💥 爆款风向标 (Global Trends & Info Gap)
- 哪些话题/技术在海外极度火爆但国内 (B站/V2EX) 尚未普及？（信息差红利）
- 哪些是"更新、更热"的刚需？

### 2. 🔄 海外 vs 国内 双向比对
- 找出那些存在巨大信息差 (高 info_gap) 的项目
- 制作一个对比表格

### 3. 🎯 商业化变现落地建议
- 针对"高信息差"的社媒热点，推荐打包哪些开源平替
- 小红书/闲鱼具体推广话术与起号建议
